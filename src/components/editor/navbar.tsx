...
export default function Navbar() {
  const [videoTitle, setVideoTitle] = useState(() => {
    // Initialize from localStorage or fallback to "Untitled video"
    return localStorage.getItem('videoTitle') || "Untitled video"
  });

  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newTitle = e.target.value;
    setVideoTitle(newTitle);
    localStorage.setItem('videoTitle', newTitle);
  };
    ...
    >
      <div className="flex items-center gap-2 pointer-events-auto h-14">
        <div className="bg-zinc-950 h-12 w-12 flex items-center justify-center rounded-md">
          <img src="https://www.remotion.pro/_next/image?url=%2Flogo-small.png&w=64&q=75" alt="logo" className="h-5 w-5" />
        </div>
      ...
      <div className="pointer-events-auto  h-14 flex items-center gap-2 justify-center">
        <div className="bg-zinc-950 px-2.5 rounded-md h-12 gap-4 flex items-center">
          <input
            ...
            className="font-medium text-sm px-1 bg-transparent outline-none border-none focus:ring-0"
            spellCheck={false}
          />
          <ResizeVideo />
        </div>
      ...
      <div className="flex items-center gap-2 pointer-events-auto h-14 justify-end">
        <div className="flex items-center gap-2 bg-zinc-950 px-2.5 rounded-md h-12">
          <Button
            ...
            onClick={() => openLink('https://github.com/TynzX')}
            size="xs"
            variant="secondary"
          >
...
const DownloadPopover = () => {
  ...
  const videoTitle = localStorage.getItem('videoTitle') || 'Untitled video';
  ...
  const handleExportTemplate = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${videoTitle}-template.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
        ...
        {downloadState.isDownloading ? (
          ...
          <>
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button onClick={handleExportTemplate} variant="secondary" className="w-full" size="xs">
                Export Template
              </Button>
            </div>
          </>
